...
Credentials are the JSON config file downloaded from developer.factset.com when
an application is registered. It carries the client id and an RSA private key
(the ``jwk`` block), so it is a secret: keep it outside the repo, or at least
...
Requires the FactSet auth helper, which is not part of the base install::
...
# Mirrors fds.sdk.utils' own requirement. Checked here so a malformed file is
# reported against the file the user actually pointed at.
...
def load_config(path: str | os.PathLike | None = None) -> dict[str, Any]:
    """Read and validate the OAuth config. The returned dict holds a private key."""
    ...
    if not isinstance(jwk, dict):
        raise FactSetConfigError(
            f"{config_path} is missing the 'jwk' block. Confirm you downloaded "
            "the confidential-client config and not the application summary."
        )
...
def confidential_token_provider(
    ...
    """Build a callable returning a bearer token, refreshing it as it expires.
    ...
    client caches the token and renews it 30 seconds before expiry, so calling
    the returned function per request is cheap.
    """
    try:
        from fds.sdk.utils.authentication import ConfidentialClient
    except ImportError as exc:  # pragma: no cover - depends on install state
        raise FactSetConfigError(
            "The FactSet auth helper is not installed. Run: "
            "pip install -r requirements-factset.txt"
        ) from exc
    ...
    def __init__(
        ...
        *,
        ...
        if token_provider is not None:
            ...
            self._token_provider = confidential_token_provider(
                config=config,
